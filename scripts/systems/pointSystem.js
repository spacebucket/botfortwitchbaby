...
$.on('command', function (event) {
    ...
    if (command.equalsIgnoreCase("gold") || command.equalsIgnoreCase($.pointname) || command.equalsIgnoreCase("bank")) {
    if (args.length >=1) {
        var action = args[0];
        ...
        if (action.equalsIgnoreCase("give") || action.equalsIgnoreCase("send")) {
            ...
            if ($var.perm_toggle == false) {
                ...
                $.say("From now on Mods will be able to use privileged gold commands!");

            } else if ($var.perm_toggle == true) {

                $var.perm_toggle = false;
                $.inidb.set('settings', 'perm_toggle', 2);
                $.say("From now on only Admins will be able to use privileged gold commands!");
            }
            ...
            $.say("Usage: '!gold give <name> <amount>' -- '!gold take <name> <amount>' -- '!gold set <name> <amount>' -- '!gold gift <name> <amount>' -- '!gold gain <amount>' -- '!gold bonus <amount>' -- '!gold name <amount>'");
            return;
        } else {
    ...
    if (command.equalsIgnoreCase("gift")) {
        ...
        if (username == sender) {
            $.say("Why would you need to gift yourself ?!");
            return;
        }
